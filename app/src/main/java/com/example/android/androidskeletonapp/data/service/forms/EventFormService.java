...
public class EventFormService {
    ...
    private final Map<String,
            Triple<ProgramStageDataElement, DataElement, TrackedEntityDataValueObjectRepository>> fieldMap;
    ...
    public Flowable<Map<String,
            Triple<ProgramStageDataElement, DataElement,
                    TrackedEntityDataValueObjectRepository>>> getEventFormFields() {
        if (d2 == null)
            ...
            return Flowable.fromCallable(() ->
                    d2.programModule().programStages.uid(eventRepository.get().program())
                            .withAllChildren().get().programStageDataElements()

            )
                    .flatMapIterable(programStageDataElements -> programStageDataElements)
                    .map(programStageDataElement -> {
                        DataElement dataElement = d2.dataElementModule().dataElements.uid(programStageDataElement.uid()).get();
                        ...
                        fieldMap.put(dataElement.uid(), Triple.of(
